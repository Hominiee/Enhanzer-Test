...
//        Thread.sleep(2000);
        ...
        //*****Check the validation using submission modal*****
        boolean isSubmissinModalPresent = driver.findElements(By.id("example-modal-sizes-title-lg")).size() > 0;

        if (!isSubmissinModalPresent) {
            System.out.println("Form submission failed due to invalid email format!");
        } else {
            System.out.println("A valid email and form submitted successfully!");
        }
        Thread.sleep(3000);
